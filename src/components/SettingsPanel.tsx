...
import { X, Layers, Palette, Music, UserRound, ZoomIn } from 'lucide-react';
...
import { useUiScaleStore, UI_SCALES } from '@/store/uiScaleStore';
...
export default function SettingsPanel({ open, onClose }: SettingsPanelProps) {
  ...
  const { scale: uiScale, setScale: setUiScale } = useUiScaleStore();
            ...
            <div className="p-5 space-y-6">
              ...
              <section>
                ...
                <div className="flex flex-wrap gap-2">
                  {UI_SCALES.map((s) => (
                    <button
                      ...
                      className={cn(
                        'px-4 py-2 rounded-xl border font-main text-sm transition-all duration-200',
                        uiScale === s
                          ? (isLight
                            ? 'bg-amber-500 text-white border-amber-500'
                            : 'bg-gold-500 text-ink-900 border-gold-400')
                ...
                <div className={cn(
                  ...
                  4K / 高分屏可调大界面（手牌、记牌器等同步放大）；手机小屏布局自动适配，不受缩放影响
                </div>
              </section>
