...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    highlights: '体验优化版本',
    added: [
      '界面缩放：4K / 高分屏可调大界面（75%~175% 七档），手牌、记牌器、弹窗等全局等比同步放大；手机小屏布局自动适配不受影响',
      '再开一局：离线对局结算后可直接以当前模式与难度开新局，无需返回主界面',
    ],
    fixed: [
      '再开一局后背景音乐停留在上一局报警的 Exciting 不切回 Gaming（切曲依赖改为随新局 gameId 变化触发）',
      '提起牌时选牌音效时有时无（高频短音效改用预加载复用池，杜绝 new Audio 的加载延迟与缓存读取失败）',
    ],
  },
  {
    version: 'v1.5.1',
    ...
    highlights: '联机修复版本',
    added: [
      'Electron 便携版打包：双击即玩的单文件 exe（内嵌前后端与运行时，免安装 Node.js），数据存于用户目录',
      '游戏分流页（game.fruitrade.cn）：「欢迎来到午夜雅集」主题入口页，斗地主 / 怀旧斗地主 / 麻将三张卡片分流跳转',
      '项目工程清理：移除打包产物与构建中间文件，瘦身约 893MB',
    ],
    fixed: [
      '联机对战：同一浏览器开多个窗口时，第二个窗口「加入房间」被识别为房主本人而不分配座位，导致房主看不到加入的玩家、开始后其他玩家无法进入对局（玩家身份改存 sessionStorage，每个标签页独立身份，同标签页刷新仍可回到对局）',
    ],
  },
  {
    version: 'v1.5.0',
